...
func (tr *BTreeG[T]) SetHint(item T, hint *PathHint) (itemToUse T, found bool) {
	if tr.locks {
		tr.mu.Lock()
		itemToUse, found = tr.setHint(item, hint)
		tr.mu.Unlock()
	} else {
		itemToUse, found = tr.setHint(item, hint)
	}
	return itemToUse, found
}
...
func (tr *BTreeG[T]) setHint(item T, hint *PathHint) (itemToUse T, found bool) {
	...
	prev, found, split := tr.nodeSet(&tr.root, item, hint, 0)
	if split {
...
func (tr *BTreeG[T]) nodeSet(cn **node[T], item T, hint *PathHint, depth int) (prev T, found bool, split bool) {
	...
	prev, found, split = tr.nodeSet(&(*n.children)[i], item, hint, depth+1)
	if split {
		if len(n.items) == tr.max {
			return tr.empty, false, true
		}
