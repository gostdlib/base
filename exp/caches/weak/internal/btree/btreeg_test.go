...
func TestGenericRandom(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		if v, ok := tr.Set(keys[i]); ok || !tr.eq(v, keys[i]) {
			t.Fatalf("expected nil")
		}
		if i%123 == 0 {
			tr.sane()
	...
	shuffleItems(keys)
	for i := 0; i < len(keys); i++ {
		if v, ok := tr.Load(keys[i]); ok || !tr.eq(v, keys[i]) {
			t.Fatalf("expected nil")
		}
		if i%97 == 0 {
			tr.sane()
...
func TestGenericCopy(t *testing.T) {
	...
	testCopyDeep = func(tr *BTreeG[testKind], parent bool) {
		...
		for i := 0; i < len(items2); i++ {
			if v, ok := tr.Set(items2[i]); ok || !tr.eq(v, items2[i]) {
				panic("!")
			}
		}
		tr.sane()
...
func TestGenericVarious(t *testing.T) {
	...
	for _, i := range randKeys(N) {
		if v, ok := tr.SetHint(i, &hint); ok || !tr.eq(v, i) {
			panic("!")
		}
	}
	for _, i := range randKeys(N) {
