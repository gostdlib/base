...
func TestPanicErrorUnwrap(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			flight := &Flight[string, int]{}

			func() {
				defer func() {
					recovered = recover()
					t.Logf("after panic(%#v) in Flight.Do, recovered %#v", tc.panicValue, recovered)
				}()

				_, _, _ = flight.Do(t.Context(), tc.name, func() (int, error) {
					panic(tc.panicValue)
				})
			}()
...
func TestDo(t *testing.T) {
	...
	v, err, _ := g.Do(t.Context(), "key", func() (string, error) {
		return "bar", nil
	})
...
func TestForget(t *testing.T) {
	...
	go func() {
		g.Do(t.Context(), "key", func() (int, error) {
			close(firstStarted)
			<-unblockFirst
			close(firstFinished)
			return 0, nil
	...
	<-firstStarted
	g.Forget(t.Context(), "key")

	unblockSecond := make(chan struct{})
	secondResult := g.DoChan(t.Context(), "key", func() (i int, e error) {
		<-unblockSecond
		return 2, nil
	})
	...
	thirdResult := g.DoChan(t.Context(), "key", func() (i int, e error) {
		return 3, nil
	})
...
func TestDoChan(t *testing.T) {
	var g Flight[string, string]
	ch := g.DoChan(t.Context(), "key", func() (string, error) {
		return "bar", nil
	})
...
func TestPanicDoChan(t *testing.T) {
	if os.Getenv("TEST_PANIC_DOCHAN") != "" {
		...
		g := new(Flight[string, struct{}])
		ch := g.DoChan(t.Context(), "", func() (struct{}, error) {
...
func TestPanicDoSharedByDoChan(t *testing.T) {
	if os.Getenv("TEST_PANIC_DOCHAN") != "" {
		...
		go func() {
			...
			g.Do(t.Context(), "", func() (struct{}, error) {
				close(blocked)
				<-unblock
...
func ExampleFlight() {
	...
	g := new(Flight[string, string])

	block := make(chan struct{})
	res1c := g.DoChan(ctx, "key", func() (string, error) {
		<-block
		return "func 1", nil
	})
	res2c := g.DoChan(ctx, "key", func() (string, error) {
		<-block
		return "func 2", nil
	})
	close(block)
